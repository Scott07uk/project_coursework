...
FRAME_SIZE = (int(720/4), int(1280/4))
DEVICE = 'cuda'
BATCH_SIZE = 12
...
class DashcamDataset(Dataset):
    ...
    for index in range(len(times) - self.prev_frames):
      ...
      if times[index + self.prev_frames] >= video_stop_time:
        for use_index in range(self.prev_frames):
          ...
          if not cv2.imwrite(output_image_name, frames[index]):
            print(f'unable to write file {output_image_name}')
  ...
  def __getitem_with_index__(self, ix, image_file_index):
    ...
    try:
      ...
      print(f'Exception thrown reading file {image_file}: {e}')
